                ...
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="dom-jqry" class="table table-striped table-bordered nowrap">
                            ...
                            <tbody>
                                @foreach($receiptVoucher as $item)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $item->bill_no }}</td>
                                    <td>{{ \Carbon\Carbon::parse($item->date)->format('d/m/Y') }}</td>
                                    <td>
                                        <a href="{{ route('superadmin.account.receiptvoucher.show', $item->voucher_id) }}" class="avtar avtar-xs btn-link-secondary"><i class="ti ti-eye f-20"></i></a>

                                        @if ($item->edit_status == 1 || $item->created_at->diffInHours(now()) <= 24)
                                            <a href="{{ route('superadmin.account.receiptvoucher.edit', $item->voucher_id) }}" class="badge bg-success">Edit NOw</a>
                                        @elseif ($item->edit_status == 2)
                                            <span class="badge bg-danger">Request Rejected</span>
                                        @elseif ($item->edit_status == 3)
                                            Request Sent
                                        @else
                                            <a href="{{ route('superadmin.account.receiptvoucher.edit.request', $item->voucher_id) }}" class="">Send Edit Request</a>
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
