      ...
      <div class="card-header">
        <div class="row align-items-center">
            ...
            <div class="col-sm-6 text-sm-end mt-3 mt-sm-0">
                <h4>Bill No: {{ $journalVoucher->bill_no }}</h4>
            </div>
          </div>
      </div>
      <div class="card-body">
        <form id="myForm" method="POST" action="{{ $url }}">
            ...
            <div class="row">
                <div class="mb-3 col-lg-6">
                    ...
                    <input type="text" name="date" placeholder="Select Date" id="date" class="form-control"
                        value="{{ old('date', $journalVoucher->date) }}">
                </div>
                <div class="mb-3 col-lg-6">
                    <label class="form-label">Narration:</label>
                    <input type="text" name="narration" class="form-control"
                        value="{{ old('narration', $journalVoucher->narration) }}">
                </div>

                <div id="journal-voucher-forms">
                    @foreach($ledgerHeads as $index => $entry)
                                    ...
                                    <div class="row">
                                        <div class="col-sm-4">
                                            ...
                                            <select name="ledger_head[]" class="form-control">
                                                ...
                                                <option value="{{ $item->ledgers_head_code }}"
                                                    {{ $entry->ledger_head == $item->ledgers_head_code ? 'selected' : '' }}>
                                        ...
                                        <div class="col-sm-4">
                                            ...
                                            <select name="crdr[]" class="form-control">
                                                ...
                                                <option value="1" {{ $entry->crdr == 1 ? 'selected' : '' }}>Credit</option>
                                                <option value="2" {{ $entry->crdr == 2 ? 'selected' : '' }}>Debit</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="mb-3 col-lg-12">
                    ...
                    <textarea name="remarks" class="form-control" rows="3">{{ old('remarks', $journalVoucher->remarks) }}</textarea>
                </div>
            </div>
