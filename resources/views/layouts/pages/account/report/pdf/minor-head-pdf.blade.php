    ...
    <table>
        ...
        <tbody>
            ...
            @foreach($groupedIncomeBalances as $key => $groupData)
                <tr>
                    @php
                        $majorHeadCode = substr($key, 0, 3); // First 4 digits
                        $minorHeadCode = substr($key, 3);    // Remaining digits

                        $majorHead = \App\Models\MajorHead::where('code', $majorHeadCode)->first();
                        $minorHead = \App\Models\MinorHead::where('code', $minorHeadCode)->where('major_head_code', $majorHead->id)->first();
                    @endphp


                    <td>{{ $key }}</td>
                    <td>{{ $minorHead->name ?? 'NA' }}</td>
