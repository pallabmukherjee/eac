...
class MinorHeadIncomeExpenditureController extends Controller {

    public function index(){
        $title = "Minor Head Income & Expenditure Summary";
        $url = route('superadmin.account.report.minorHeadIncomeExpenditureReport');
    ...
    public function report(Request $request) {
        ...
        $groupedIncomeBalances = $yearlyIncomeBalances->groupBy(function($item) {
            return substr($item->ledgers_head_code	, 0, 5); // Group by the first 3 digits
        });
        ...
        $pdf = PDF::loadView('layouts.pages.account.report.pdf.minor-head-pdf', compact('groupedIncomeBalances', 'groupedExpenditureBalances', 'selectedYear', 'website'))
        ->setOption('margin-top', 0)
        ->setOption('margin-left', 0)
        ->setOption('margin-right', 0)
        ->setOption('margin-bottom', 0);

        return $pdf->stream('minor_head_INCOME_EXPENDITURE.pdf');
    }
}
